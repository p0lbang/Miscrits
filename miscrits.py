...
targetBypass = True #.............# set to True to make everyone a target for capture
...
def LXVI_readImage(
    ...
    if numerical:
        read = reader.recognize(numpy.array(img), allowlist='0123456789', blocklist='-~()')
    else:
        ...
        if (miscrit in targets or targetBypass) :
            ...
            if autoCatch:
                ...
                if isinstance(catchButton, Point):
                    toCatch = int(LXVI_readImage(region=(int(catchButton.x) - 17, int(catchButton.y) + 13, 18, 22))) <= catchStandard
                if toCatch:
                    playSound(dance)
                    catchMode()
                    return
...
def catchMode():
    ...
    while not caught:
        if not checkActive():
            print(F"\033[AMinimized while trying to catch {Fore.YELLOW}{miscrit}{Fore.LIGHTBLACK_EX}, concluding process...")
            conclude()

        if LXVI_locateCenterOnScreen("miscripedia.png", confidence=0.8) is None:
            if LXVI_locateCenterOnScreen("closebtn.png", 0.85) is not None:
                print(f"\033[A{Fore.WHITE}Target miscrit {Fore.YELLOW}{miscrit}{Fore.WHITE} died. Failed to catch.{Fore.LIGHTBLACK_EX}\n")
                return
        ...
        if (
            ...
            if action == 0:
                ...
                pyautogui.moveTo(toClick)
                print(f"\033[A{Fore.WHITE}Time to kill {Fore.YELLOW}{miscrit}{Fore.WHITE}, I guess.{Fore.LIGHTBLACK_EX}")
                pyautogui.moveRel(-45 + 160 * 1, 0)
                pyautogui.leftClick()
                pyautogui.moveTo(toClick)
            
    print(f"\033[A{Fore.YELLOW}{miscrit}{Fore.WHITE} has been caught.{Fore.LIGHTBLACK_EX}")
